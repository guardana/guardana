...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.15 \
...
docker run --rm -v "$PWD:/work" \
  ...
  ghcr.io/guardana/guardana:0.15 \
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.15 scan /work || true
```
