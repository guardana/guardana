...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.18 \
...
docker run --rm -v "$PWD:/work" \
  ...
  ghcr.io/guardana/guardana:0.18 \
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.18 scan /work || true
```
