...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.21 \
...
docker run --rm -v "$PWD:/work" \
  ...
  ghcr.io/guardana/guardana:0.21 \
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.21 scan /work || true
```
