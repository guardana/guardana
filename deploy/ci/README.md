...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.12 \
...
docker run --rm -v "$PWD:/work" \
  ...
  ghcr.io/guardana/guardana:0.12 \
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.12 scan /work || true
```
