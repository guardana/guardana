...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.11 \
...
docker run --rm -v "$PWD:/work" \
  ...
  ghcr.io/guardana/guardana:0.11 \
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.11 scan /work || true
```
