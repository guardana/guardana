...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.20 \
...
docker run --rm -v "$PWD:/work" \
  ...
  ghcr.io/guardana/guardana:0.20 \
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.20 scan /work || true
```
