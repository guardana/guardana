...
| **Rule fixtures** | `guardana rule test 'acme.*'` | Runs the samples a rule declares — a positive, a negative and an **inconclusive** one — against scripted doubles, sending nothing. The third is the point: a rule that cannot fire is caught by the first sample, a rule that fires on everything by the second, and **a rule that cannot say "I could not tell" by nothing at all**. A rule with no fixtures, or with no inconclusive fixture, exits `2` — a command built to disprove false greens cannot print "ok" over an empty set of cases. `--write-corpus` turns the fixtures into a labelled set `calibrate` can measure a judge against. |
| **Pack manifest** | `guardana pack validate` | Reads the `guardana-pack.yaml` inside an installed package and answers two questions: can this build load it — the declared `extension_api` range, refused in **both** directions with different messages — and does it register everything it promises. The missing direction is the one that matters: a pack declaring a check it does not register leaves a team believing something runs that does not. `provides:` names all four extension groups; schema **2** added `taxonomies:`, so a pack that ships only a control catalogue can describe itself at all — before it, `provides:` had to list something and none of the three things it could list were what such a pack shipped. Versioned and migratable: a schema 1 manifest loads, migrated forward **in memory** and never by rewriting the author's file, and `pack validate` prints that it happened. Guardana's own pack ships one and goes through the same door. |
| **Extension lock** | `guardana pack lock`, `--check` | Pins every installed extension by what it **is**, not by what its package is called: rules by `Rule.digest()` — their declaration, hashed — so a pack that sharpens a corpus inside a patch release is visible, which a version pin is not. Evaluators and targets are pinned by id (Python has no declaration to hash, and a digest invented from a class name would claim to detect a change it cannot see); catalogues by a digest over the references they register; the distribution version sits beside all of it as the coarse pin that covers what none of the others can. Drift is reported in both directions — a rule that vanished is coverage a team still believes they have, one that appeared is a check nobody reviewed. Extensions from a package with no manifest are recorded as `unlocked` and said out loud, because a lock silent about them would read as a fully pinned repository that is not one. |
...
exits non-zero rather than letting that pass. **`--record` writes the measurement
where a run can carry it**, and a profile's `calibrations:` block is what picks it
up — so a saved run states how honest its judge's confidence was, on which corpus and
when, instead of leaving `calibration` null as every run did before 0.18. An
unreliable measurement is refused rather than recorded: writing a figure the command
printed a caveat about into a run's evidence would put noise where a reader takes a
measurement. Record what it measured under
