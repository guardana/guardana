...
jobs:
  guardana:
    ...
    steps:
      ...
      - uses: guardana/guardana@v0.19   # moving tag → latest 0.19.x
        # with:
...
| **0.19** *(current)* | Documentation generated onto guardana.dev from this repository's own prose and its own registry, with a **rule explorer** whose every filter is a pre-rendered page — because `script-src 'none'` is a claim a visitor checks, not a default. Plus a round-trip gate on the run manifest, which found the `deployment` block written on every run and read back on none |
