...
from guardana.core.diff import ChangeKind, RunContext, compare
...
def _kinds(before: ScanResult, after: ScanResult) -> list[ChangeKind]:
    return [c.kind for c in compare(before, after, before_context=_CTX, after_context=_CTX).changes]
...
def test_a_rule_that_stopped_running_is_lost_coverage() -> None:
    ...
    changes = compare(before, after, before_context=_CTX, after_context=_CTX).changes
...
def test_a_rule_that_started_running_is_gained_coverage() -> None:
    changes = compare(
        _run(ran=(_RULE,)), _run(ran=(_RULE, _OTHER)), before_context=_CTX, after_context=_CTX
    ).changes
...
def test_an_unchanged_run_reports_nothing_and_counts_the_checks() -> None:
    ...
    diff = compare(before, after, before_context=_CTX, after_context=_CTX)
    assert diff.changes == ()
...
def test_a_rule_whose_definition_changed_is_flagged_on_the_change() -> None:
    ...
    diff = compare(
        _run(),
        _run(findings=(_finding(),)),
        before_context=RunContext(_ENDPOINT, {_RULE: "aaaa", _OTHER: "cccc"}),
        after_context=RunContext(_ENDPOINT, {_RULE: "bbbb", _OTHER: "cccc"}),
    )
